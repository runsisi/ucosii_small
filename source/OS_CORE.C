...
INT8U  RUNSISI_CODE /*const*/   OSMapTbl[]   = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
...
INT8U RUNSISI_CODE /*const*/  OSUnMapTbl[] = {
...
void  OS_TaskIdle (void *pdata1) RUNSISI_LARGE_REENTRANT
{
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr;
#endif    
    ...
    pdata1 = pdata1;                               /* Prevent compiler warning for not using 'pdata'     */
    for (;;) {
        OS_ENTER_CRITICAL();
        OSIdleCtr++;
        OS_EXIT_CRITICAL();
